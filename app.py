...
def get_phone_number_with_email_id(email):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT phone, Name FROM employee_master WHERE email = %s", (email,)
        )
        row = cursor.fetchone()
        cursor.close()
        conn.close()
...
try:
    ...
    with st.sidebar:
        ...
        from_date, to_date = st.date_input(
            "📅 Select Date Range", [default_from, default_to]
        )
    ...
    agent_data = (
        ...
        else dispo_df[dispo_df["agent_name"] == selected_agent]
    )
    ...
    selected_dispo = st.selectbox(
        "🎯 Select a Disposition", dispo_counts["disposition_name"].tolist()
    )

    st.markdown("---")
    st.subheader(f"📞 Call Logs for: **{selected_dispo}**")

    selected_qids = agent_data[agent_data["disposition_name"] == selected_dispo][
        "q_id"
    ].tolist()
    ...
    selected_call_display = st.selectbox(
        "🎧 Select a Call to Play Recording:", ["Select..."] + call_options
    )
